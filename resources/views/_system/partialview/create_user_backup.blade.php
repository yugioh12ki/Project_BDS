...
<form id="addUserForm" action="{{ route('admin.users.store') }}" method="POST">
    ...
    @if(request('showCreateModal'))
        <input type="hidden" name="redirect_to_property" value="1">
    ...
    <div class="mb-3">
        ...
        <select class="form-control" id="userRole" name="role" {{ request('showCreateModal') ? 'readonly' : '' }}>
            @if(request('showCreateModal'))
